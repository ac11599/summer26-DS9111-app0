...
# --- Visualization Page ---
    ...
    with tab2:
        ...
        cat_data = df.groupby(cat_x)[cat_y].sum()
...
#             profile = ProfileReport(df,title="Ads Revenue Report",explorative=True,minimal=True)
#             st_profile_report(profile)

#         export = profile.to_html()
#         st.download_button(label="📥 Download full Report",data=export,file_name="ads_revenue_report.html",mime='text/html')
...
#         "Select Features (X)", list_var, default=list_var)
#     target_selection = st.sidebar.selectbox(
#         "Select target variable (Y))", list_var)
